...
def delivery_receive(askFile, seperatefileSize, cacheFileName):
	...
	while data:
		if data == 'new file':
			...
			while receiveFlag and data != 'new file' and data:				
				file_cache.write(data)
				data = sock_receiver.recv(BUFFER_SIZE)
			'''
		
			# same as the while before, but use kodo
			size = int(seperatefileSize)
			while receiveFlag and data != 'new file' and len(data) > 0:
				...
				for blockSeque in range(0, numOfBlock, 1):
					...
					# select the block size, the last block's size is different
					if blockSeque != numOfBlock-1:
						decoder = kododecoder(blockSize)	
					else:
						lastBlockSize = size - (numOfBlock-1)*blockSize
						decoder = kododecoder(lastBlockSize)	
					# receiver the data until the decoder is complete
					while not decoder.is_complete():
						lossFlag = random.sample(lossRange, 1)
						packet = sock_receiver.recv(BUFFER_SIZE)
						if lossFlag[0]:								
					...
					# the last block we need only lastBlockSize byte data, kodo add '\x00' in the end
					tempdata = decoder.copy_from_symbols()
					if blockSeque != numOfBlock-1:
						file_cache.write(tempdata)
					else:
						file_cache.write(tempdata[0:lastBlockSize])
					# send to server: finish information
					for temp in range(0,10,1):
						sock_sender.sendto(str(blockSeque), address)
					# with this information, the client will not receiver the old packet 
					# without this info, in 4. receive phase the client will receive the data in 3. phase firstly
					# because of latency 
					data = sock_receiver.recv(BUFFER_SIZE)
					while data != 'begin':
						data = sock_receiver.recv(BUFFER_SIZE)
					...
					#for temp in range(0,10,1):
					#	sock_sender.sendto(returnSendData, address)
				
				print 'decoder is is_complete\n'
				break
